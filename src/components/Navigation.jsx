...
const Navigation = ({ isVisible }) => {
  ...
  return (
    <header
      className={`fixed top-5 left-1/2 -translate-x-1/2 w-auto z-50 transition-transform duration-300 ${
              ...
              <li key={link.name}>
                {link.name === 'Projects' ? (
                  <button
                    ...
                    className="block px-4 py-1.5 text-sm font-medium rounded-lg transition-colors duration-300 text-gray-300 hover:bg-white/5 hover:text-white"
                  >
                    {link.name}
                  </button>
                ) : (
